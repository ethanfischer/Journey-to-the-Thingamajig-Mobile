...
class FlxTilemap extends FlxObject
{
	 ...
	 */
	public var customTileRemap:Array<Dynamic><Int>;
	 ...
	 */
	private var _randomIndices:Array<Dynamic><Int>;
	private var _randomChoices:Array<Dynamic><Array<Dynamic><Int>>;
	/**
	 ...
	 */
	private var _tileObjects:Array<Dynamic><FlxTile>;
	 ...
	 */
	private var _rectIDs:Array<Dynamic><Int>;
	#end
	...
	public function new()
	{
		...
		_buffers = new Array<Dynamic><FlxTilemapBuffer>();
	 ...
	 * 
	 * @param	MapData      	A string of comma and line-return delineated indices indicating what order the tiles should go in, or an Array<Dynamic> of Int. YOU MUST SET widthInTiles and heightInTyles manually BEFORE CALLING loadMap if you pass an Array<Dynamic>!
	...
	public function loadMap(MapData:Dynamic, TileGraphic:Dynamic, TileWidth:Int = 0, TileHeight:Int = 0, AutoTile:Int = 0, StartingIndex:Int = 0, DrawIndex:Int = 1, CollideIndex:Int = 1):FlxTilemap
	{
		...
		if (Std.is(MapData, String))
		{
			...
			_data = new Array<Dynamic><Int>();
			var columns:Array<Dynamic><String>;
			var rows:Array<Dynamic><String> = MapData.split("\n");
		...
		// Data is already set up as an Array<Dynamic><Int>
		// DON'T FORGET TO SET 'widthInTiles' and 'heightInTiles' manually BEFORE CALLING loadMap() if you pass an Array<Dynamic><Int>!
		else if (Std.is(MapData, Array<Dynamic>))
		{
		...
		_tileObjects = new Array<Dynamic><FlxTile>();
		...
		_rects = new Array<Dynamic><Rectangle>();
	 ...
	 * 
	 * @param	mappings		Array<Dynamic> of ints for remapping tiles. Ex: [7,4,12] means "0-->7, 1-->4, 2-->12"
	 * @param	randomIndices	(optional) Array<Dynamic> of ints indicating which tile indices should be randmoized. Ex: [7,4,12] means "replace tile index of 7, 4, or 12 with a randomized value"
	 ...
	 */
	public function setCustomTileMappings(mappings:Array<Dynamic><Int>, randomIndices:Array<Dynamic><Int> = null, randomChoices:Array<Dynamic><Array<Dynamic><Int>> = null, randomLambda:Void->Float = null):Void
	{
	 ...
	 */
	public function getData(Simple:Bool = false):Array<Dynamic><Int>
	{
		if (!Simple)
		{
			return _data;
		...
		var data:Array<Dynamic><Int> = new Array<Dynamic>();
	 ...
	 * @return	An Array<Dynamic> of FlxPoints, containing all waypoints from the start to the end.  If no path could be found, then a null reference is returned.
	 */
	public function findPath(Start:FlxPoint, End:FlxPoint, Simplify:Bool = true, RaySimplify:Bool = false, WideDiagonal:Bool = true):Array<Dynamic><FlxPoint>
	{
		...
		var distances:Array<Dynamic><Int> = computePathDistance(startIndex, endIndex, WideDiagonal);
		...
		var points:Array<Dynamic><FlxPoint> = new Array<Dynamic><FlxPoint>();
		...
		var path:Array<Dynamic><FlxPoint> = [];
	 ...
	 * 
	 * @param	Dynamic			The object being tested.
	...
	/**
	 * Checks if the Dynamic overlaps any tiles with any collision flags set,
	 * and calls the specified callback function (if there is one).
	 * Also calls the tile's registered callback if the filter matches.
	 * 
	 * @param	Dynamic				The FlxObject you are checking for overlaps against.
	 ...
	 */
	public function overlapsWithCallback(Dynamic:FlxObject, ?Callback:FlxObject->FlxObject->Bool, FlipCallbackParams:Bool = false, ?Position:FlxPoint):Bool
	{
		...
		var selectionX:Int = Math.floor((Dynamic.x - X) / _scaledTileWidth);
		var selectionY:Int = Math.floor((Dynamic.y - Y) / _scaledTileHeight);
		var selectionWidth:Int = selectionX + Math.ceil(Dynamic.width / _scaledTileWidth) + 1;
		var selectionHeight:Int = selectionY + Math.ceil(Dynamic.height / _scaledTileHeight) + 1;
		...
		while (row < selectionHeight)
		{
			...
			while (column < selectionWidth)
			{
				...
				overlapFound = ((Dynamic.x + Dynamic.width) > tile.x)  && (Dynamic.x < (tile.x + tile.width)) && 
				               ((Dynamic.y + Dynamic.height) > tile.y) && (Dynamic.y < (tile.y + tile.height));
				...
				if (tile.allowCollisions != FlxObject.NONE)
				{
					if (Callback != null)
					{
						...
						{
							overlapFound = Callback(Dynamic, tile);
						}
						else
						{
							overlapFound = Callback(tile, Dynamic);
						}
					}
				}
				...
				if (overlapFound)
				{
					if ((tile.callbackFunction != null) && ((tile.filter == null) || Std.is(Dynamic, tile.filter)))
					{
						tile.mapIndex = rowStart + column;
						tile.callbackFunction(tile, Dynamic);
					}
	 ...
	 * @return	An Array<Dynamic> with a list of all map indices of that tile type.
	 */
	public function getTileInstances(Index:Int):Array<Dynamic><Int>
	{
		var array:Array<Dynamic><Int> = null;
		var i:Int = 0;
	 ...
	 * @return	An Array<Dynamic> with a list of all the coordinates of that tile type.
	 */
	public function getTileCoords(Index:Int, Midpoint:Bool = true):Array<Dynamic><FlxPoint>
	{
		var array:Array<Dynamic><FlxPoint> = null;
		...
		while (i < l)
		{
			if (_data[i] == Index)
			{
				...
				if (array == null)
				{
					array = new Array<Dynamic><FlxPoint>();
				}
				array.push(point);
			}
	 ...
	 * @param	Callback			The function to trigger, e.g. lavaCallback(Tile:FlxTile, Dynamic:FlxObject).
	...
	public function updateFrameData():Void
	{
		if (cachedGraphics != null && _tileWidth >= 1 && _tileHeight >= 1)
		{
			...
			#if FLX_RENDER_TILE
			_rectIDs = new Array<Dynamic><Int>();
	...
	private function drawTilemap(Buffer:FlxTilemapBuffer, Camera:FlxCamera):Void
	{
		...
		var currDrawData:Array<Dynamic><Float> = drawItem.drawData;
	 ...
	 */
	private function simplifyPath(Points:Array<Dynamic><FlxPoint>):Void
	{
	 ...
	 * @param	Points		An array of FlxPoint nodes.
	 */
	private function raySimplifyPath(Points:Array<Dynamic><FlxPoint>):Void
	{
	 ...
	 * @return	A Flash Array<Dynamic> of FlxPoint nodes.  If the end tile could not be found, then a null Array<Dynamic> is returned instead.
	 */
	public function computePathDistance(StartIndex:Int, EndIndex:Int, WideDiagonal:Bool, StopOnEnd:Bool = true):Array<Dynamic><Int>
	{
		...
		var distances:Array<Dynamic><Int> = new Array<Dynamic><Int>(/*mapSize*/);
		...
		var distance:Int = 1;
		var neighbors:Array<Dynamic><Int> = [StartIndex];
		var current:Array<Dynamic><Int>;
		...
		{
			current = neighbors;
			neighbors = new Array<Dynamic><Int>();
	 ...
	 * 
	 * @param	Data	A Flash Array<Dynamic> of distance information.
	 * @param	Start	The tile we're on in our walk backward.
	 * @param	Points	A Flash Array<Dynamic> of FlxPoint nodes composing the path from the start to the end, compiled in reverse order.
	 */
	private function walkPath(Data:Array<Dynamic><Int>, Start:Int, Points:Array<Dynamic><FlxPoint>):Void
	{
