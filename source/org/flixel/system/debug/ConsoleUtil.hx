...
class ConsoleUtil
{
	 ...
	 */
	public static function callFunction(Function:Dynamic, Args:Array<Dynamic><Dynamic>):Bool
	{
		try
		{
		...
		catch (e:ArgumentError)
		{
			if (e.errorID == 1063)
			{
				...
				if (expected < Args.length) 
				{
					...
					var shortenedArgs:Array<Dynamic><Dynamic> = Args.slice(0, expected);
					// Try again
	...
	/**
	 * Searches for the Command typedef for a given Alias within a Command Array<Dynamic>.
	 * 
	 * @param	Alias		The Alias to search for.
	 * @param	Commands	The array of commands to search through
	 * @return	The Command typdef - null if none was found.
	 */
	public static function findCommand(Alias:String, Commands:Array<Dynamic><Command>):Command
	{
	 ...
	 */
	@:generic
	public static function attemptToCreateInstance<T>(ClassName:String, type:Class<T>, ?Params:Array<Dynamic><String>):Dynamic
	{
		if (Params == null)
			Params = [];
	 ...
	 * @param	Dynamic	Starting point for the search, has to contain the first object / variable of the first param
	 * @return	A PathToVarible typedef, or null.
	 */
	public static function resolveObjectAndVariable(ObjectAndVariable:String, Dynamic:Dynamic):PathToVariable
	{
		var searchArr:Array<Dynamic><String> = ObjectAndVariable.split(".");
		
		if (searchArr.length == 1)
		{
			return { object: Dynamic, variableName: ObjectAndVariable };
		}
		
		var variableName:String = searchArr.join(".");
		
		if (!Reflect.isObject(Dynamic)) 
		{
			FlxG.log.error("'" + FlxStringUtil.getClassName(Dynamic, true) + "' is not a valid Object");
			return null;
		}
		
		// Searching for property...
		var l:Int = searchArr.length;
		var tempObj:Dynamic = Dynamic;
	...
	{
		var splitString:Array<Dynamic><String> = ObjectAndVariable.split(".");
	 ...
	 */
	public static function getInstanceFieldsAdvanced(cl:Class<Dynamic>, numSuperClassesToInclude:Int = 0):Array<Dynamic><String>
	{
		var fields:Array<Dynamic><String> = Type.getInstanceFields(cl);
		if (numSuperClassesToInclude >= 0)
		{
			var curClass = Type.getSuperClass(cl);
			var superClasses:Array<Dynamic><Class<Dynamic>> = [];
			...
			{
				var superFields:Array<Dynamic><String> = Type.getInstanceFields(superClasses[i]);
	...
	/**
	 * Attempts to parse a String into a Bool, returns 
