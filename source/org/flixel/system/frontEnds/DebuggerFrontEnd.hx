...
{	
	 ...
	 */
	public var toggleKeys:Array<Dynamic><String>;
	#end
	 ...
	 * 
	 * @param	Dynamic	The object to track
	 * @param	WindowTitle	Title for the tracker window, uses the Dynamic's class name by default
	 */
	public function track(Dynamic:Dynamic, ?WindowTitle:String):Window
	{
		#if !FLX_NO_DEBUG
		if (Tracker.objectsBeingTracked.indexOf(Dynamic) == -1)
		{
			var profile = Tracker.findProfile(Dynamic);
			if (profile == null)
			{
				FlxG.log.error("FlxG.debugger.track(): Could not find a tracking profile for this object of class '" + FlxStringUtil.getClassName(Dynamic, true) + "'."); 
				return null;
			}
			else 
			{
				return FlxG.game.debugger.addWindow(new Tracker(profile, Dynamic, WindowTitle));
			}
		}
		else 
		{
