...
class FlxFlicker implements IFlxDestroyable
{
	 ...
	 */
	public static function flicker(Dynamic:FlxObject, Duration:Float = 1, Interval:Float = 0.04, EndVisibility:Bool = true, ForceRestart:Bool = true, ?CompletionCallback:FlxFlicker->Void, ?ProgressCallback:FlxFlicker->Void):Void
	{
		if (isFlickering(Dynamic))
		{
			if (ForceRestart)
			{
				stopFlickering(Dynamic);
			}
			else
			{
		...
		flicker.start(Dynamic, Duration, Interval, EndVisibility, CompletionCallback, ProgressCallback);
		_boundObjects[Dynamic] = flicker;
	}
	
	/**
	* Returns whether the object is flickering or not.
	* @param  Dynamic The object to test.
	*/
	public static function isFlickering(Dynamic:FlxObject):Bool
	{
		return _boundObjects.exists(Dynamic);
	}
	...
	*/
	public static function stopFlickering(Dynamic:FlxObject):Void
	{
		var boundFlicker:FlxFlicker = _boundObjects[Dynamic];
	...
	*/
	private function start(Dynamic:FlxObject, Duration:Float, Interval:Float, EndVisibility:Bool, ?CompletionCallback:FlxFlicker->Void, ?ProgressCallback:FlxFlicker->Void):Void
	{
		object = Dynamic;
		duration = Duration;
		interval = Interval;
