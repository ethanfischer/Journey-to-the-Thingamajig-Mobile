...
class FlxBitmapTextField extends FlxSprite
{
	...
	#if FLX_RENDER_BLIT
	private var _preparedTextGlyphs:Array<Dynamic><BitmapData>;
	private var _preparedShadowGlyphs:Array<Dynamic><BitmapData>;
	private var _preparedOutlineGlyphs:Array<Dynamic><BitmapData>;
	#else
	private var _drawData:Array<Dynamic><Float>;
	private var _bgDrawData:Array<Dynamic><Float>;
	#end
	...
	private function updateBitmapData():Void 
	{
		...
		// Get words
		var lines:Array<Dynamic><String> = preparedText.split("\n");
		var i:Int = -1;
		var j:Int = -1;
		...
		while (++i < lines.length) 
		{
			...
			{
				lineComplete = false;
				var words:Array<Dynamic><String> = [];
	...
	#if FLX_RENDER_BLIT
	private function clearPreparedGlyphs(PxGlyphs:Array<Dynamic><BitmapData>):Void
	{
		if (PxGlyphs != null)
		{
			for (bmd in PxGlyphs)
